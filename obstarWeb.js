...
var log_file_err = fs.createWriteStream(__dirname + "/web_error.log", {
  flags: "a"
});
...
var USERS = c.MYSQL
  ? require("mysql").createPool(require("./lib/webMysql.js").info)
  : 0;
///
var LEADERBOARD = [];
var SHOP = { HIDE: 1 };
...
if (USERS) {
  ...
  if (c.DB.LB) {
    let updateLB = () => {
      USERS.query(
        'SELECT score, name, tank, gm, DATE_FORMAT(date, "%d-%m-%Y") AS date FROM wrs ORDER BY score DESC',
        function(err, leader) {
          if (err) throw err;
          LEADERBOARD = leader;
        }
  ...
  if (c.DB.SHOP) {
    let updateShop = () => {
      USERS.query("SELECT class, id, label, price FROM shop", function(
        err,
        shop
      ) {
        if (err) throw err;
...
app.get("*", function(request, respond) {
  ...
  /// get the acc///
  if (USERS && c.DB.AC) {
    USERS.query("SELECT * FROM acc WHERE userKey LIKE ?", [KEY], function(
      err,
      result,
      fields
    ) {
      if (result && result.length && result[0]) {
        ///   THERE IS AN ACC  ///
        USERS.query(
          "UPDATE acc SET lastConnection = NOW() WHERE userKey = ?",
          [KEY],
          function(err) {
            if (err) {
              throw err;
        ...
        respond.cookie("obstarkey", newkey, {
          expires: new Date(253402300000000),
          sameSite: "Lax"
        });
        let sendData = {
...
app.post("/userData", function(req, res) {
  if (USERS && c.DB.ACC) {
    USERS.query(
      "SELECT userData, coins FROM acc WHERE userKey = ?",
      [req.body.userKey],
...
app.post("/buy", function(req, res) {
  ...
  if (
    ...
    !req.body.class ||
    !SHOP[req.body.class] ||
    !SHOP[req.body.class][req.body.id]
  ) {
    delete SHOPPER[req.body.userKey];
    res.status(200).send("no obj");
    return;
  }
  var obj = SHOP[req.body.class][req.body.id],
    objC = req.body.class,
    objId = req.body.id;
  USERS.query(
    "SELECT userData, coins FROM acc WHERE userKey = ?",
    ...
    function(err, result, fields) {
      if (result.length && result[0].userData) {
        ...
        if (user.own && user.own[objC] && user.own[objC][objId]) {
          ...
          USERS.query(
            "UPDATE acc SET userData = ?, coins = ? WHERE userKey = ?",
            [stringUser, user.coins, req.body.userKey],
            function(err) {
              if (err) throw err;
              delete SHOPPER[req.body.userKey];
            }
...
app.post("/play", function(request, respond) {
  ...
  respond.cookie("preference", pref, {
    expires: new Date(253402300000000),
    sameSite: "Strict"
  });
