...
var USERS = c.MYSQL
  ? require("mysql").createPool(require("./lib/AlexMysql.js").info)
  : 0;
if (USERS) {
  ...
  function income(socket, packet) {
    ...
    switch (data.type) {
      ...
      case "init": {
        ...
        socket.id = Controller.askConnection(
          data.data,
      ...
      case "keydown": {
        socket.main.request -= 0.5;
        let tank = Controller.getPlayer(socket.id);
        if (!Controller.getPlayer(socket.id)) {
          break;
        }
...
var CONFIG = {
  BOTS: [
    function() {
      if (isNaN(this.path)) {
        this.path =
          CONFIG.BOT_PATHS[parseInt(Math.random() * CONFIG.BOT_PATHS.length)];
      }
      if (this.stillLvl) {
        this.upgrade(
          CONFIG.BOT_UPS[this.path.up ? this.path.up : 0][this.stillLvl]
        );
      }
      ...
      if (!this.DETEC) {
        this.DETEC = new Detector(
          this,
          this.x,
          this.y,
          this.screen / 2,
          ["Player", "Objects", "Bullet"],
          0,
          1
        );
      ...
      if (!this.botMod) {
        ...
        if (
          all.Objects.length + all.Player.length + all.Bullet.length > 0 &&
          !this.running
        ) {
          ...
          if (this.DETEC.select.constructor.name == "Player") {
            ...
            attack +=
              ((Math.min(
                Math.pow(other.xp / tresh.attackxpBase, 1.4),
                tresh.attackxpMax
              ) /
                tresh.attackxpDivide) *
                Math.max(1, this.level / other.level) *
                (1 / (1 + other.hp / tresh.attackHp)) *
                (1 / (1 + this.DETEC.dis / tresh.attackDis)) *
                this.hp) /
              tresh.attack;
          }
          this.run = run;
      ...
      switch (this.botMod) {
        case "farm": {
          ...
          for (let obj of this.DETEC.selectAll.Objects) {
            let dis = Math.sqrt(
              Math.pow(this.x - obj.x, 2) + Math.pow(this.y - obj.y, 2) * 2
            );
            if (dis < oldDis) {
              oldDis = dis;
              selected = obj;
          ...
          if (
            oldDis >
            CONFIG.botThreshold.farmDis * len + this.size + selected.size
          ) {
        ...
        case "run": {
          ...
          for (let bull of this.DETEC.selectAll.Bullet) {
            let dis =
              this.screen /
              Math.sqrt(
                Math.pow(this.x - bull.x, 2) + Math.pow(this.y - bull.y, 2) * 2
              ) /
              CONFIG.botThreshold.runDis;
          ...
          for (let bull of this.DETEC.selectAll.Player) {
            ...
            x +=
              ((bull.x * bull.hp) / CONFIG.botThreshold.runHp) *
              bull.damage *
              dis;
            y +=
              ((bull.y * bull.hp) / CONFIG.botThreshold.runHp) *
              bull.damage *
              dis;
          }
          if (!med) {
          ...
          let dis = Math.sqrt(
            Math.pow(this.x - x, 2) + Math.pow(this.x - x, 2)
          );
          this.autoDir = Math.atan2(y - this.y, x - this.x);
          dir = Math.PI + this.autoDir;
          dir +=
            (this.spin * Math.PI * Math.min(1, Math.sqrt(dis / this.screen))) /
            1.9;
          break;
        }
        case "attack": {
          ...
          this.autoDir = Math.atan2(
            other.y + (other.vec.y * dis) / 12 - this.y,
            other.x + (other.vec.x * dis) / 12 - this.x
          );
          dir =
            (this.spin * Math.PI * Math.min(1, 100 / dis)) / 2.5 + this.autoDir;
          break;
        }
        case "search":
        default: {
      ...
      this.dir = Math.atan2(
        Math.sin(this.dir) +
          (Math.sin(this.autoDir) - Math.sin(this.dir)) * 0.3,
      ...
      if (Math.abs(Math.PI * 0.5 - vdir) <= tresh) {
        motion.y -= len;
      }
      if (Math.abs(Math.PI * 1.5 - vdir) <= tresh) {
        motion.y += len;
      }
      if (Math.abs(Math.PI - hdir) <= tresh) {
        motion.x -= len;
      }
      if (hdir <= tresh) {
        motion.x += len;
  ...
  BOSS: [
    [
      function() {
        if (!this.DETEC) {
          ...
          for (let n of this.DETEC.selectAll.Player) {
            let dis =
              Math.sqrt(
                Math.pow(n.x - this.x, 2) + Math.pow((n.y - this.y) / 0.5625, 2)
              ) / n.level;
            if (dis < n.screen / 30) {
        ...
        let motion = new Vec(0.15, 0).rotate(
          Math.atan2(this.y, this.x) -
            Math.PI * Math.min(1, dis / this.map.width)
        );
        this.vec.add(motion);
        this.vec.x *= FRICTION;
        this.vec.y *= FRICTION;
      ...
      function() {
        ...
        ///
        this.motion();
        if (this.detected.length || Math.random() > 0.6) {
          this.up.BPene = this.detected.length * 0.9;
          this.shoot();
        }
        ///
      },
  ...
  PETS: [
    function(play) {
      this.showDir = Math.atan2(
        play.y + play.inputs.mouse_y - this.y,
        play.x + play.inputs.mouse_x - this.x
      );
      if (!this.delay) {
      ...
      ////
      this.dir = Math.atan2(
        play.y + play.vec.y * 3 + this.pos.y - this.y,
        play.x + play.vec.x * 3 + this.pos.x - this.x
      );
...
class Main {
  constructor() {
    ...
    this.chat = function() {
        ...
        if (client[id].chat) {
          for (let c of clientMess) {
            if (c && c.gr == clientMess[client[id].chat].gr) {
              c.mess.push([
                clientMess[client[id].chat].c +
                  clientMess[client[id].chat].name,
                data
              ]);
            }
          }
      ...
      function com(id, data) {
        ...
        switch (arr[0]) {
          ...
          case "/name": {
            if (client[id].chat) {
              clientMess[client[id].chat].mess.push([
                "",
                clientMess[client[id].chat].gr
              ]);
            }
            break;
          }
          ...
          case "/color": {
            if (client[id].chat) {
              if (
                arr[1].length == 6 &&
                arr[1].match(/[0-9A-Fa-f]/g).length == 6
              ) {
    ...
    if (USERS && c.DB.LB) {
      ...
      USERS.query(
        "SELECT score, id FROM wrs ORDER BY score c.DESC LIMIT " +
          this.scoresLimit,
        function(err, lead) {
          if (err) throw err;
          this.scores = new Array(lead.length).fill(0).map((x, y) => {
            return { id: lead[y].id, score: lead[y].score };
          });
  ...
  askConnection(data, ip) {
    ...
    var connect = (data, ip) => {
      ...
      switch (data.gm) {
        ...
        case "2team":
          ...
          break;
        default:
          Controller.clients[clientId] = "ERR_GAMEMODE";
          return;
      }
      /////
    ...
    ///
    if (USERS && c.DB.ACC) {
      USERS.query("SELECT * FROM acc WHERE userKey = ?", [data.key], function(
        err,
        user,
        fields
      ) {
  ...
  command(id, com) {
    ...
    if (!com.length || typeof this.clients[id] !== "object") {
      return;
    }
    com = com.split(" ");
    if (com[0] == "disconnect") {
    ...
    if (com[0] == "connect") {
      if (com[1] && /*!this.devs[com[1]] &&*/ !this.clients[id].dev) {
        USERS.query(
          "SELECT * FROM devs WHERE password = ?",
          [com[1]],
      ...
      switch (this.devs[this.clients[id].dev]) {
        case 3: {
          if (com[0] == "player" && !isNaN(parseInt(com[1]))) {
            p = this.server[this.clients[id].GM][this.clients[id].sId].INSTANCE
              .players[parseInt(com[1])];
            if (!p) {
          ...
          switch (com[0]) {
            ...
            case "invisible": {
              ...
              p.dev.invisible =
                com[1] == "on" ? 1 : com[1] == "off" ? 0 : p.dev.invisible;
              if (p.dev.invisible) {
                p.alpha = 0;
              } else {
                p.alpha = 1;
              ...
              return (
                "mode invisible " +
                (!p.dev.invisible ? "activated" : "deactivated")
              );
              break;
            }
            case "collision": {
              if (!p.dev) return;
              p.dev.ghost =
                com[1] == "on" ? 0 : com[1] == "off" ? 1 : p.dev.ghost;
              return (
                "collision " + (!p.dev.ghost ? "activated" : "deactivated")
              );
              break;
            }
            case "mess": {
            ...
            case "broadcast": {
              ...
              let c = com.join(" ");
              if (c.length) {
                for (let i of this.server[this.clients[id].GM][
                  this.clients[id].sId
                ].INSTANCE.players) {
                  if (i && isNaN(i) && !i.bot) {
                    i.mess.push(c);
            ...
            case "mapResize": {
              if (!isNaN(com[1]) && !isNaN(com[2])) {
                this.server[this.clients[id].GM][
                  this.clients[id].sId
                ].newMap = {
                  width: parseInt(com[1]),
                  height: parseInt(com[2])
                };
                return `Map resized to w: ${parseInt(com[1])} h: ${parseInt(
                  com[2]
                )}`;
              }
              break;
          ...
          switch (com[0]) {
            ...
            case "getBots": {
              ...
              for (let i of this.server[this.clients[id].GM][
                ...
                if (i && i.bot) {
                  let m = "",
                    id = i.id.oId.toString(),
                    name = i.name.replace(/([^a-z0-9]+)/gi, "-");
            ...
            case "getObj": {
              ...
              for (let i of this.server[this.clients[id].GM][
                ...
                if (
                  typeof i === "object" &&
                  dis(
                    p.x + p.inputs.mouse_x - i.x,
                    p.y + p.inputs.mouse_y - i.y
                  ) < i.size
                ) {
                  let m = "",
                    id = i.id.oId.toString();
            ...
            case "tp": {
              if (!isNaN(com[1]) && !isNaN(com[2])) {
                p.x = parseInt(com[1]);
                p.y = parseInt(com[2]);
                return `Position set to x: ${parseInt(p.x)} y: ${parseInt(
                  p.y
                )}`;
              }
              break;
        ...
        case 1: {
          switch (com[0]) {
            ...
            case "setHp": {
              p.hp = Math.max(
                0,
                Math.min(p.maxHp, isNaN(com[1]) ? p.maxHp : parseInt(com[1]))
              );
              return (
                "Hp set to " + (isNaN(com[1]) ? p.maxHp : parseInt(com[1]))
              );
              break;
            }
          }
  ...
  newServer(gameMode) {
    ...
    switch (gameMode) {
      case "ffa":
        s = new Sffa(this.server[gameMode].length);
        break;
      case "2team":
        s = new S2team(this.server[gameMode].length);
        break;
      case "4team":
        s = new S4team(this.server[gameMode].length);
        break;
      case "boss":
        s = new Sboss(this.server[gameMode].length);
        break;
    }
    console.log(
      cc.Bright +
    ...
    if (this.scores && score) {
      for (let i = this.scores.length - 1; i >= 0; i--) {
        if (
          ...
          (i == 0 ? true : score <= this.scores[i - 1].score)
        ) {
          this.highestScoreId++;
          this.scores.splice(i, 0, { score: score, id: this.highestScoreId });
          USERS.query(
            "INSERT INTO wrs VALUES(NULL,?,?,?,?,?,NOW())",
            [name, score, tank, gm, key],
            function(err) {
              if (err) throw err;
            }
          );
          if (this.scores.length > this.scoresLimit) {
            USERS.query(
              "DELETE FROM wrs WHERE id = ?",
              [this.scores[this.scores.length - 1].id],
              function(err) {
                if (err) throw err;
              }
            );
  ...
  disconnect(id, ip) {
    ...
    if (!p.dev && isNaN(p.dev))
      this.insertLB(tank.name, tank.xp, tank.class, p.GM, p.key);
...
class Sffa {
  ...
  generate(go = 1) {
    ...
    if (RNG < 1) {
      ...
      if (obj[0] < obj.max0) {
        this.createObj("sqr", 0);
        obj[0]++;
      }
      if (obj[1] < obj.max1 && Math.random() < 0.26) {
        this.createObj("sqr", 1);
        obj[1]++;
      }
    }
    ///TRIANGLE///
    if (RNG < 0.7) {
      ...
      if (obj[0] < obj.max0) {
        this.createObj("tri", 0);
        obj[0]++;
      }
      if (obj[1] < obj.max1 && Math.random() < 0.26) {
        this.createObj("tri", 1);
        obj[1]++;
      }
    }
    ///PENTAGONE///
    if (RNG < 0.5) {
      ...
      if (obj[0] < obj.max0) {
        this.createObj("pnt", 0);
        obj[0]++;
      }
      if (obj[1] < obj.max1 && Math.random() < 0.2) {
        this.createObj("pnt", 1);
        obj[1]++;
      }
    }
    ///BULL///
  ...
  createObj(type, pos) {
    ...
    for (let i = 0; i <= this.INSTANCE.objs.length; i++) {
      if (!this.INSTANCE.objs[i]) {
        this.INSTANCE.objs[i] = new Objects(
          type,
          ppp,
  ...
  createAi() {
    for (let i = 10; i < 20; i++) {
      let bot = new Player(
        ...
        0,
        CONFIG.BOT_NAMES[
          parseInt(Math.random() * (CONFIG.BOT_NAMES.length - 1))
        ],
        1,
        this.XPLVL
      );
  ...
  update() {
    ...
    if (Math.abs(this.map.width - this.newMap.width) > 0.1) {
      this.map.width += (this.newMap.width - this.map.width) * 0.1;
    } else {
      this.map.width = this.newMap.width;
    }
    if (Math.abs(this.map.height - this.newMap.height) > 0.1) {
      this.map.height += (this.newMap.height - this.map.height) * 0.1;
    } else {
    ...
    this.timestamp++;
    let qt = new quadTree(
      -this.map.width / 2 - 1000,
      -this.map.height / 2 - 1000,
      this.map.width + 2000,
      this.map.height + 2000,
      6
    );
    this.leader = [];
    for (let c in this.INSTANCE) {
      for (let j in this.INSTANCE[c]) {
          ...
          if (this.leader.length) {
            for (let l = Math.min(this.leader.length - 1, 9); l >= 0; l--) {
              if (this.leader.length < 9) {
                ...
                this.leader[l].xp < i.xp &&
                (!l || this.leader[l - 1].xp >= i.xp)
              ) {
                this.leader.splice(l, 0, i);
                this.leader.pop();
                break;
        ...
        if (i.destroy == 1) {
          ...
          if (c == "bullets") {
            this.INSTANCE[c][j] = c.KEEP_PLACE;
            continue;
          }
          delete this.INSTANCE[c][j];
        } else {
      ...
      for (let obj of this.INSTANCE[c]) {
        ...
        let collide = qt.query(
          ...
          },
          {
            x: obj.x,
            y: obj.y,
            r: (obj.DETEC && obj.DETEC.enabled ? obj.DETEC.size : obj.size) * 2
          }
        );
        for (let i in collide) {
          ...
          if (
            obj.id.oId == other.id.oId &&
            obj.constructor.name == other.constructor.name
          ) {
            continue;
          }
          let dis = Math.sqrt(
              ...
              if (objCLASS == "Bullet") {
                if (other.destroy && other.prize) {
                  if (this.INSTANCE.players[obj.origine.oId]) {
                    ...
                    this.INSTANCE.players[obj.origine.oId].coins +=
                      other.coinReward || 0;
                    if (
                      otherCLASS == "Player" &&
                      !this.INSTANCE.players[obj.origine.oId].bot
                    ) {
                      this.INSTANCE.players[obj.origine.oId].mess.push(
                        "You killed " + other.name
                      );
                    }
                  }
                }
              ...
              if (otherCLASS == "Bullet" && obj.prize) {
                if (obj.destroy) {
                  if (this.INSTANCE.players[other.origine.oId]) {
                    ...
                    this.INSTANCE.players[other.origine.oId].coins +=
                      obj.coinReward || 0;
                    if (
                      objCLASS == "Player" &&
                      !this.INSTANCE.players[other.origine.oId].bot
                    ) {
                      this.INSTANCE.players[other.origine.oId].mess.push(
                        "You killed " + obj.name
                      );
                    }
                  }
    ...
    for (let id in this.INSTANCE.players) {
      ...
      var x = player.x - player.screen / 2 - 200,
        y = player.y - (player.screen / 2) * 0.5625 - 200;
      var w = player.screen + 400,
        h = player.screen * 0.5625 + 400;
    ...
    let x,
      ...
      xp = force
        ...
        ? Math.min(
            tank.xp,
            parseInt(
              Math.pow(tank.xp / (mXp / Math.pow(mXp * 0.6, 1 / 0.9)), 0.9)
            )
          )
        : mXp * 0.6;
    while (1) {
      ...
      if (dis > 1100) {
        dis = Math.sqrt(
          Math.pow(this.map.width / 4 - x, 2) +
            Math.pow(this.map.height / 4 - y, 2)
        );
        if (dis > 800) {
          dis = Math.sqrt(
            Math.pow(-this.map.width / 4 - x, 2) +
  ...
  getBuffer(id) {
    ...
    buff.head.level = !lvl
      ...
      : lvl +
        Math.max(
          Math.min(1, (xp - arr[lvl - 1]) / (arr[lvl] - arr[lvl - 1])),
          0
        );
    ///
    buff.main = {
    ...
    for (let i of RAW.rest) {
      ...
      switch (obj.constructor.name) {
        ...
        case "Bullet": {
          if (obj.origine.oId == RAW.main.id.oId) {
            let raw = new Int8Array(
              Controller.encodeInst("Instance", {
                len: 21,
                construc: "Bullets",
                id: obj.id.oId,
                states: [!!obj.pet * 1, 0, 0, 0, 0, 0, 0],
...
class S2team {
  constructor(id) {
    ...
    for (let i = 1; i <= this.droneQt; i++) {
      ...
      bull.id = {
        GM: this.gm,
        sId: this.id,
        oId: this.INSTANCE.bullets.length
      };
      bull.team = 0;
      bull.ox = bull.x;
      bull.oy = bull.y;
  ...
  createBoss() {
    for (let i = 0; i <= this.maxPlayer; i++) {
      if (typeof this.INSTANCE.players[i] === "undefined" && !this.boss) {
        ...
        this.INSTANCE.players[i].mess.push(
          "Tremble at the sight of the " + CONFIG.BOSS[0][2] + " !"
        );
      }
    }
  }
    ...
    for (let c in this.INSTANCE) {
      for (let obj of this.INSTANCE[c]) {
        ...
        for (let i in collide) {
          ...
          if (objCLASS == "Detector" && otherCLASS == "Detector") {
            continue;
          }
          if (obj.id.oId == other.id.oId && objCLASS == otherCLASS) {
            continue;
          }
          let dis = Math.sqrt(
            Math.pow(other.x - obj.x, 2) + Math.pow(other.y - obj.y, 2)
          ...
          if (
            (isNaN(other.getPlace) || isNaN(obj.getPlace)) &&
            other.team != obj.team
          ) {
            if (obj.DETEC && obj.DETEC.enabled) {
              if (dis <= obj.DETEC.size + other.size) {
  ...
  respawn(id, force = 0, bot = 0) {
    ...
    let x = tank.team
      ? this.map.width / 2 - this.baseSize * Math.random()
      : -this.map.width / 2 + this.baseSize * Math.random();
    ...
    let xp = force
      ? tank.xp
      : tank.xp < mXp
      ? parseInt(Math.pow(tank.xp / (mXp / Math.pow(mXp * 0.6, 1 / 0.8)), 0.8))
      : mXp * 0.6;
    let newTank = new Player(tank.id, x, y, tank.name, tank.team, this.XPLVL);
    if (bot) {
      newTank.motion = CONFIG.BOTS[0].bind(newTank);
  ...
  ask(data) {
    ...
    for (let i = 0; i <= this.maxPlayer; i++) {
      if (typeof this.INSTANCE.players[i] === "undefined" && index == -1) {
        ...
        this.INSTANCE.players[i] &&
        !isNaN(this.INSTANCE.players[i].team)
      ) {
    ...
    if (index >= 0) {
      ...
      let tank = new Player(
        ...
        0,
        name,
        team1 == team0 ? parseInt(Math.random() + 0.5) : team0 < team1 ? 0 : 1,
        this.XPLVL
      );
      if (pet) {
        tank.pet = pet;
...
class Player {
  constructor(id, x, y, name, team, xpLvl) {
    this.XPLVL = xpLvl;
    this.mlx =
      this.XPLVL[this.XPLVL.length - 3] /
      Math.pow(this.XPLVL[this.XPLVL.length - 3], 1 / 1.8);
    this.BUFF = {
      timestamp: -1
    };
    this.extraView = 0;
  ...
  shoot() {
    ...
    for (let r in CLASS[this.class].canons) {
      if (typeof this.shootTimer[r] === "undefined") {
        this.shootTimer[r] = 0;
      }
      ...
      if (can.autoDir) {
        if (this.DETEC.select) {
          ...
          if (
            !other.destroy &&
            other.alpha &&
            dis <= CLASS[this.class].DETEC.maxDis
          ) {
            autoDir = Math.atan2(
              other.y + (other.vec.y * dis) / 12 - this.y,
              other.x + (other.vec.x * dis) / 12 - this.x
      ...
      ///
      if (
        (this.inputs.e || this.inputs.mouseL || can.auto) &&
        ...
        if (reload == Math.floor(can.offTime * reloadMax)) {
          ...
          let len =
            can.canonLength * 0.93 * ra -
            this.up.BSpeed * can.speed * exitSpeed * 2;
          ...
          let Bull = new Bullet(
            ...
            x,
            y,
            dir + Math.random() * can.rand - can.rand / 2,
            this.up.BSpeed * can.speed,
            exitSpeed
          );
      ...
      case 7:
        ...
        for (let i in this.up) {
          ...
          switch (i) {
            ...
            case "HpUp":
              this.maxHp += 110;
              this.hp = parseInt(this.hp * (this.maxHp / (this.maxHp - 100)));
              break;
            case "BodyDam":
              this.damage += 1.8;
              break;
  ...
  collision(other, option = {}) {
    ...
    if (option.type) {
      switch (option.type) {
        case "god":
          this.vec.add(
            new Vec(this.x - other.x, this.y - other.y)
              .norm()
              .multiply(new Vec(0.6, 0.6))
          );
          return;
          break;
      }
    ...
    switch (other.constructor.name) {
      case "Player":
        this.vec.add(
          new Vec(this.x - other.x, this.y - other.y)
            .norm()
            .multiply(new Vec(0.3, 0.3))
        );
        if (option.noDam || this.shield) {
          break;
        }
        ...
        break;
      case "Objects":
        let len = this.vec.length() < 0.5 ? 2 : 0.5;
        this.vec.add(
          new Vec(this.x - other.x, this.y - other.y)
            .norm()
            .multiply(new Vec(len, len))
        );
        if (
          this.necro &&
          other.type == "sqr" &&
          ...
          let Bull = new Bullet(
            ...
            this.up.BSpeed * this.necro.speed,
            0
          );
      ...
      case "Bullet":
        ...
        this.vec.add(
          new Vec(this.x - other.x, this.y - other.y)
            .norm()
            .multiply(new Vec(other.weight / 3, other.weight / 3))
        );
        if (this.shield) {
          return;
        }
        ...
        if (this.hp <= 0) {
          this.dead = c.DEAD_DELAY;
          this.murder = ["players", other.origine];
          this.destroy = c.DES;
        }
        break;
    }
    if (this.alpha < 1 && !this.dev.invisible) {
      this.alpha = Math.min(
        1,
        this.alpha + ((oldHp - this.hp) / this.maxHp) * 5
      );
    }
  }
  update() {
    ...
    if (this.xp != this.oldXp) {
      ...
      if (this.xp < this.XPLVL[this.XPLVL.length - 3]) {
        this.prize = parseInt(
          Math.min(
            this.XPLVL[this.XPLVL.length - 3],
            Math.pow(this.xp / this.mlx, 1.8)
          )
        );
      } else {
        this.prize = parseInt(
          this.XPLVL[this.XPLVL.length - 3] +
            (this.xp - this.XPLVL[this.XPLVL.length - 3]) / 10
        );
      }
    }
    if (
    ...
    if (this.dev.stick) {
      let obj =
        Controller.server[this.id.GM][this.id.sId].INSTANCE[this.dev.stick[0]][
          this.dev.stick[1]
        ];
...
class Objects {
  constructor(type, pos, id, map) {
    ...
    this.coinReward = parseInt(Math.random() + 0.02);
    this.type = type;
    this.id = id;
    this.size = 20;
    ...
    switch (pos) {
      case -1:
        while (1) {
          this.x =
            this.marge +
            Math.random() * (map.width - this.marge * 2) -
            map.width / 2;
          this.y =
            this.marge +
            Math.random() * (map.height - this.marge * 2) -
            map.height / 2;
          let dis = Math.sqrt(Math.pow(this.x, 2) + Math.pow(this.y, 2));
          if (dis > 1000) {
            dis = Math.sqrt(
              Math.pow(map.width / 4 - this.x, 2) +
                Math.pow(map.height / 4 - this.y, 2)
            );
            if (dis > 700) {
              dis = Math.sqrt(
                Math.pow(-map.width / 4 - this.x, 2) +
      ...
      default:
        ...
        this.y = Math.min(
          map.height / 2 - this.marge,
          Math.max(
            -map.height / 2 + this.marge,
            pos[1] + Math.cos(dir) * (Math.random() * pos[2])
          )
        );
        this.pos = 1;
        break;
    ...
    switch (this.type) {
      ...
      case "pnt":
        this.size = 42;
        this.hp = 190;
        this.prize = 100 + parseInt(Math.random() * 100);
        this.maxspeed = 0.08;
        this.weight = 4;
        this.damage = 5;
        break;
  ...
  collision(other, option = {}) {
    let len = this.vec.length() * this.weight < 0.4 ? 2 : 0.4;
    switch (other.constructor.name) {
      case "Player":
        if (
          other.necro &&
          this.type == "sqr" &&
          other.droneCount < CLASS[other.class].maxDrone + other.upNb[1]
        ) {
          this.destroy = 1;
          return;
        }
      ...
      case "Bullet":
        if (other.necro && this.type == "sqr") {
          let play =
            Controller.server[other.origine.GM][other.origine.sId].INSTANCE
              .players[other.origine.oId];
        ...
        this.hp -=
          (option.pene > 1 ? option.pene : option.pene / 2) * other.damage;
        this.hit = 2;
        if (this.hp <= 0) {
          this.destroy = c.DES;
        }
    ...
    if (this.DETEC) {
      if (this.DETEC.select) {
        if (this.DETEC.select.destroy || this.DETEC.select.god) {
          ...
          let v = new Vec(0.28, 0).rotate(
            Math.atan2(
              this.DETEC.select.y - this.y,
              this.DETEC.select.x - this.x
            )
          );
          this.vec.add(v);
          this.DETEC.enabled = 0;
        ...
        Math.sqrt(
          Math.pow(this.x - this.rx, 2) + Math.pow(this.y - this.ry, 2)
        ) > 120
      ) {
        let v = new Vec(0.28, 0).rotate(
          Math.atan2(this.ry - this.y, this.rx - this.x)
        );
        this.vec.add(v);
      } else {
        this.DETEC.enabled = 1;
...
class Bullet {
      ...
      switch (option.type) {
        case "god":
          ...
          this.vec.add(
            new Vec(this.x - other.x, this.y - other.y)
              .norm()
              .multiply(new Vec(this.speed * 2 + 1, this.speed * 2 + 1))
          );
          return;
          break;
      }
      ...
      switch (other.constructor.name) {
        case "Player":
          ...
          this.vec.add(
            new Vec(this.x - other.x, this.y - other.y)
              .norm()
              .multiply(new Vec(this.weight, this.weight))
          );
        ...
        case "Objects":
          ...
          if (this.necro && other.type == "sqr") {
            let play =
              Controller.server[this.origine.GM][this.origine.sId].INSTANCE
                .players[this.origine.oId];
            if (play.droneCount < CLASS[play.class].maxDrone + play.upNb[1]) {
              play.droneCount++;
              let Bull = new Bullet(
                play.id,
                ...
                other.y,
                Math.random() * Math.PI * 2,
                play.up.BSpeed * play.necro.speed,
                0
              );
              ...
              Controller.server[play.id.GM][play.id.sId].createBullet(
                Bull,
                play
              );
              return;
        ...
        case "Bullet":
          if (other.origine.oId == this.origine.oId) {
            if (
              (parseInt(this.type) == 1 || parseInt(this.type) == 3) &&
  ...
  update() {
    ...
    if (!this.alone) {
      play =
        Controller.server[this.origine.GM][this.origine.sId].INSTANCE.players[
          this.origine.oId
        ];
    ...
    switch (this.type) {
      ...
      case 1: {
        ...
        ///
        if (!this.DETEC) {
          this.DETEC = new Detector(play, this.x, this.y, 300, [
            "Player",
            "Objects"
          ]);
          this.DETEC.team = this.team;
        ...
        if (play.inputs.mouseR) {
          ...
          if (this.DETEC.select) {
            ...
            let playdis = Math.sqrt(
              Math.pow(other.x - play.x, 2) + Math.pow(other.y - play.y, 2)
            );
            if (
              dis < 300 &&
              !other.destroy &&
              playdis < play.screen / 4 &&
              other.alpha
            ) {
          ...
          let playDis = Math.sqrt(
            Math.pow(this.x - play.x, 2) + Math.pow(this.y - play.y, 2)
          );
          if (playDis < play.size * 3.5) {
            ...
            let dir = Math.atan2(
              play.y +
                Math.sin(play.autoDir * 2 + this.comingDir) * play.size * 3 -
                this.y,
              play.x +
                Math.cos(play.autoDir * 2 + this.comingDir) * play.size * 3 -
                this.x
            );
            this.dir = dir;
            break;
        ...
        if (playDis < play.size * 3) {
          ...
          let dir = Math.atan2(
            play.y +
              ...
              this.y,
            play.x +
              Math.cos(play.autoDir * 2 + this.comingDir) * play.size * 2.5 -
              this.x
          );
          this.dir = dir;
          break;
      ...
      case 1.2: {
        ...
        ///
        if (!this.DETEC) {
          this.DETEC = new Detector(play, this.x, this.y, 1400, [
            "Player",
            "Objects"
          ]);
          this.DETEC.team = this.team;
      ...
      case 1.4: {
        ...
        if (this.DETEC.select) {
          ...
          let basedis = Math.sqrt(
            Math.pow(other.x - this.ox, 2) + Math.pow(other.y - this.oy, 2)
          );
        ...
        if (baseDis < 320) {
          ...
          let dir = Math.atan2(
            this.oy + Math.sin(this.autoDir + this.comingDir) * 300 - this.y,
      ...
      case 3.1: {
        ...
        if (play.detected.length >= 1) {
          ...
          for (let n of play.detected) {
            let dis = Math.sqrt(
              Math.pow(n.x - this.x, 2) + Math.pow(n.y - this.y, 2)
            );
            if (dis <= minDis) {
              minDis = dis;
              tar = n;
        ...
        if (playDis < play.size * 4) {
          this.speed = Math.max(this.speed * 0.99, 0.05);
          if (Math.random() > 0.9995) {
            this.comingDir += Math.PI * 0.8;
            this.speed = this.maxspeed * 2;
          }
          let dir = Math.atan2(
            play.y + Math.sin(this.comingDir) * this.randPos - this.y,
...
class Detector {
  ...
  collision(other, option = {}) {
    if (this.all) {
      if (
        this.type.includes(other.constructor.name) &&
        other.alpha &&
        !other.shield
      ) {
    ...
    if (!this.self) {
      if (
        other.constructor.name == this.from.constructor.name &&
        other.id.oId == this.from.id.oId
      ) {
        return;
      }
    }
