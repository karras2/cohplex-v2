(function(window) {
  ...
  var Drawings = {
    canons: [
      (ctx, config, param, i) => {
        let c = config.canons[i],
          r = param.size / CONST.SIZE;
        if (c.hidden) {
          return;
        }
      ...
      (ctx, config, param, i) => {
        ...
        ///
        ctx.moveTo(
          (c.height * recoil - c.openlength) * r,
          (c.offx - c.width / 2) * r
        );
        ctx.lineTo(0, (c.offx - c.width / 2) * r);
        ctx.lineTo(0, (c.offx + c.width / 2) * r);
    ...
    body: [
      ...
      (ctx, config, param) => {
        ...
        roundRect(
          ctx,
          -param.size * config.body.width,
          -param.size * config.body.height,
          param.size * 2 * config.body.width,
          param.size * 2 * config.body.height,
          1
        );
        ctx.closePath();
    ...
    obj: {
      ...
      bull: (ctx, $0, $1, $2) => {
        offcan.can.height = offcan.can.width =
          $1 * 2 * RATIO * CONST.OFFCAN + 2;
  ...
  General["updateRatio"] = () => {
    RATIO = Math.max(
      Global.canW / Game.screen,
      Global.canH / (Game.screen * 0.5625)
    );
    UIRATIO = Math.max(Global.canW / 1920, Global.canH / (1920 * 0.5625)); //*(CONST.RESOLUTION);
  };
  General["ease-in-out"] = function(t, e = 5) {
    return t <= 0.5 ? Math.pow(2 * t, e) / 2 : 1 - Math.pow(2 * (1 - t), e) / 2;
  };
  General["isMouse"] = (x, y, w, h, r = 1) => {
    let mouse_x = Global.mouse_x / r,
      mouse_y = Global.mouse_y / r;
    return mouse_x >= x && mouse_y >= y && mouse_x <= x + w && mouse_y <= y + h;
  };
  General["isMouseCirc"] = (x, y, r) => {
    return (
      Math.sqrt(
        Math.pow(Global.mouse_x - x, 2) + Math.pow(Global.mouse_y - y, 2)
      ) <= r
    );
  };
  General["color"] = {
  ...
  class Tank {
    ...
    update() {
      ...
      if (this.canDir.length == this.canDdir.length) {
        for (let i in this.canDir) {
          this.canDdir[i] = Math.atan2(
            Math.sin(this.canDdir[i]) +
              (Math.sin(this.canDir[i]) - Math.sin(this.canDdir[i])) * 0.3,
            Math.cos(this.canDdir[i]) +
              (Math.cos(this.canDir[i]) - Math.cos(this.canDdir[i])) * 0.3
          );
        }
      } else {
      ...
      if (this.shield) {
        ...
        if (this.SH.lapse == 6) {
          this.SH.body = [
            General.color.shade(C[this.color][0], 1.1),
            C[this.color][1]
          ];
      ...
      ///
      if (this.hp < 1) {
        this.hpAlpha = Math.min(0.8, this.hpAlpha + 0.05);
      } else {
    ...
    draw(ctx) {
      ...
      let w = can.width / CONST.OFFCAN,
        h = can.height / CONST.OFFCAN;
    ...
    drawUi(ctx) {
      ...
      ctx.scale(
        1 / CONST.OFFCAN / CONST.RESOLUTION,
        1 / CONST.OFFCAN / CONST.RESOLUTION
      );
      ctx.drawImage(
        this.off.can,
  ...
  class Obj {
    ...
    draw(ctx) {
      ...
      if (this.type == "bull") {
        let can = General["drawBullet"].draw(ctx, {
          size: this.size,
          type: 0,
          color: this.hitted > 1 ? "hit" : this.color
        });
        return;
      }
      Drawings["obj"][this.type](
        ctx,
        this.hitted > 1 ? C.hit : C[this.color],
        this.dsize,
        this.dir
      );
    }
  ...
  class Bullet {
    ...
    update() {
      ...
      if (this.ddir != this.dir) {
        this.ddir = Math.atan2(
          Math.sin(this.ddir) +
            (Math.sin(this.dir) - Math.sin(this.ddir)) * 0.2,
  ...
  function Run() {
    ...
    General["drawTank"] =
      ...
      (() => {
        ...
        function setCoord(config) {
          let middleX = 0,
            middleY = 0,
            canSize = CONST.SIZE / 2 + CONST.LINEWIDTH;
          let marge = 2;
          ///
          if (config.canons) {
            for (let c of config.canons) {
              ...
              ///
              let cos = Math.cos(c.offdir),
                sin = Math.sin(c.offdir);
              middleX +=
                cos * Math.max(0, c.height - CONST.SIZE / 2) + sin * c.offx;
              middleY +=
                sin * Math.max(0, c.height - CONST.SIZE / 2) + cos * c.offx;
            }
    ...
    General["drawBullet"] = (() => {
      function canDraw(param) {
        ...
        can.width = can.height =
          (param.size * 2 + CONST.LINEWIDTH + 2) * CONST.OFFCAN;
        ctx.setTransform(
          CONST.OFFCAN,
          0,
          0,
    ...
    var User = new (function() {
      ...
      this.update = function() {
        ...
        let len = 0.31 / 2;
        let FRICTION = 0.95;
        if (Global.inputs.w || Global.inputs.ArrowUp) {
          motionDir[0] -= len;
        }
        if (Global.inputs.s || Global.inputs.ArrowDown) {
          motionDir[0] += len;
        }
        if (Global.inputs.a || Global.inputs.ArrowLeft) {
          motionDir[1] -= len;
        }
        if (Global.inputs.d || Global.inputs.ArrowRight) {
          motionDir[1] += len;
        }
        let ddir = Math.atan2(motionDir[0], motionDir[1]);
        let llen = Math.min(
          Math.sqrt(motionDir[0] * motionDir[0] + motionDir[1] * motionDir[1]),
          len
        );
        this.predic.xspeed += Math.cos(ddir) * llen;
        this.predic.xspeed *= FRICTION;
        this.predic.yspeed += Math.sin(ddir) * llen;
        this.predic.yspeed *= FRICTION;
        this.predic.x += this.predic.xspeed;
        this.predic.y += this.predic.yspeed;
        let tolen = Math.sqrt(
          Math.pow(this.predic.x, 2) + Math.pow(this.predic.y, 2)
        );
        ...
        this.dir = Math.atan2(
          Global.mouse_y -
            Global.winH / 2 -
            this.predic.y -
            this.dvy / CONST.SMOOTH,
          Global.mouse_x -
            Global.winW / 2 -
            this.predic.x -
            this.dvx / CONST.SMOOTH
        );
        ...
        if (this.hp < 1) {
          this.hpAlpha = Math.max(0, Math.min(0.8, this.hpAlpha + 0.05));
        } else {
          this.hpAlpha = Math.max(0, Math.min(0.8, this.hpAlpha - 0.01));
        }
        for (let i in this.recoil) {
          if (this.recoil[i] > 0 && this.recoil[i] < 0.07) {
            this.recoil[i] += (0.075 - this.recoil[i]) * 0.3;
      ...
      this.draw = function() {
        ctx.translate(
          this.dx + this.dvx + this.predic.x,
          this.dy + this.dvy + this.predic.y
        );
        ctx.globalAlpha = this.alpha;
    ...
    General["background"] =
      ...
      (() => {
        return (posx, posy, tileSize) => {
          let h = Game.screen * 0.5625 * RATIO;
          ///
          ...
          ctx.beginPath();
          for (
            let x = -(posx * RATIO - Global.canW / 2) % ts;
            x <= Game.screen * RATIO + (posx % ts);
            x += ts
          ) {
            ctx.moveTo(x, 0);
            ctx.lineTo(x, h);
          ...
          for (
            let y = -(posy * RATIO - Global.canH / 2) % ts;
            y <= h + (posy % ts);
            y += ts
          ) {
            ctx.moveTo(0, y);
    ...
    General["Ui"] = new (function() {
      ...
      this.ST = (() => {
        ...
        ///
        barc = "#fbe048 ";
        bardkc = General["color"].shade(barc, 0.5);
        barMarge = 9;
        barW = 42;
        barH = 16;
        barRad = 9;
        ...
        function bar(lvl) {
          ...
          ctx.fillRect(
            0,
            0,
            lvl * barW + Math.max(0, lvl - 1) * barMarge,
            barH
          );
          ctx.fillStyle = barc;
          ctx.fillRect(
            0,
            0,
            parseInt(lvl) * barW + parseInt(Math.max(0, lvl - 1)) * barMarge,
            barH
          );
          ctx.restore();
        }
      ...
      this.UP = (() => {
        ...
        function drawAll(tankClass, states, max = 6) {
          ...
          let w = max * (W + marge);
          can.height =
            (plusRad * 2 + lw * 2 + 4) * states.length + 20 + 18 + 6 * 4;
          can.height *= R;
        ...
        function redraw(state, nb, isMouse, colored = 1) {
          ...
          if (data.odd) {
            ...
            /////
            let plusC = data.isfull
              ? "#a8a8a8"
              : isMouse
              ? General.color.shade(C.up[state], 1.4)
              : C.up[state];
            ctx.beginPath();
            ...
            let plusC = data.isfull
              ? "#a8a8a8"
              : isMouse
              ? General.color.shade(C.up[state], 1.6)
              : C.up[state];
            ctx.beginPath();
      ...
      this.TNK = (() => {
        ...
        function mouseOn(c, is) {
          if (ALL.Class[c].is != is) {
            ...
            ctx.closePath();
            ctx.fillStyle = is
              ? General.color.shade(C.class[ALL.Class[c].id], 1.3)
              : C.class[ALL.Class[c].id];
            ctx.fill();
            ctx.lineJoin = "round";
            ctx.lineWidth = lw;
        ...
        function getImage(c) {
          ...
          ctx.drawImage(
            ...
            tank.can.width * 2,
            tank.can.width,
            tank.can.height - tank.can.width * 2,
            0,
            tank.can.width - (2 + lw + fnt) * R - 6,
            tank.can.width,
            tank.can.height - tank.can.width * 2
          );
          return tank.img;
        ...
        function draw(all) {
          ...
          for (let i in all) {
            ...
            let text =
              one.name +
              " - " +
              parseInt((one.xp / Math.pow(10, zeros * 3)) * 10) / 10 +
              " " +
              ext[zeros];
            if (!ALL.leads[text]) {
            ...
            ctx.drawImage(
              ...
              0,
              -3,
              ALL.leads[text].can.width / R,
              ALL.leads[text].can.height / R
            );
          }
          ///
      ...
      this.END = (() => {
        ...
        function setTitle(name, xp) {
          ...
          let font = "700 ";
          let text =
            "score: " + xp.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        ...
        function setTank(tank) {
          ...
          ctx.save();
          ctx.translate(
            img.can.width / 2 - img.mX,
            img.can.height / 2 - img.mY
          );
          ...
          ctx.strokeText(
            tank,
            img.can.width / 2 - m / 2,
            img.can.height - parseInt(20 * R) / 2
          );
          ctx.fillText(
            tank,
            img.can.width / 2 - m / 2,
      ...
      this.map = function() {
        ...
        ctx.fillStyle = "#222222";
        ctx.scale(
          CONST.OFFCAN * CONST.RESOLUTION,
          CONST.OFFCAN * CONST.RESOLUTION
        );
        ctx.beginPath();
        ctx.arc(
          -this.MAP.size / 2 + (User.x / Game.width) * this.MAP.size,
          this.MAP.size / 2 + (User.y / Game.height) * this.MAP.size,
          this.MAP.cursSize,
          0,
          Math.PI * 2
        );
      ...
      this.upgrade = function() {
        ...
        if (
          this.UP.isShowing ||
          Global.inputs.u ||
          parseInt(this.END.offy + 0.1)
        ) {
          this.UP.show = Math.min(
            this.dead ? 1 : 1.8,
            this.UP.show + (this.dead ? this.UP.speed * 0.6 : this.UP.speed)
          );
        } else {
        ...
        this.UP.init(
          User.class,
          CLASS[User.class].ups
            ? CLASS[User.class].ups
            : TanksConfig.defaultUps,
          6
        );
        ///
        ...
        ctx.setTransform(
          ...
          0,
          UIRATIO,
          Global.canW / 2 + User.predic.x,
          Global.canH / 2 + User.predic.y
        );
        ctx.scale(
          1 / CONST.OFFCAN / CONST.RESOLUTION,
          1 / CONST.OFFCAN / CONST.RESOLUTION
        ...
        if (SHOW) {
          ...
          if (this.still) {
            ctx.drawImage(
              ...
              -this.UP.logo.sw / 2,
              -this.UP.logo.sh / 2 -
                92 * SHOW * CONST.RESOLUTION * CONST.RESOLUTION * CONST.OFFCAN,
              this.UP.logo.sw, //w
              this.UP.logo.sh //h
            );
          }
          for (let i in this.UP.up) {
            ...
            let j = General.isMouseCirc(
              (Global.canW / 2 +
                User.predic.x +
                ((up.odd ? offx : -offx) / CONST.OFFCAN / CONST.RESOLUTION) *
                  UIRATIO) /
                CONST.RESOLUTION,
              (Global.canH / 2 +
                User.predic.y +
                ((offy / CONST.OFFCAN) * UIRATIO) / CONST.RESOLUTION) /
                CONST.RESOLUTION,
              12 * UIRATIO
            );
            if (j) {
      ...
      this.tanks = function() {
        if (
          this.classLvl != this.TNK.classLvl ||
          User.class != this.TNK.class
        ) {
          ...
          for (let i = 0; i < this.classLvl; i++) {
            if (CLASS_TREE[i][User.class]) {
              this.TNK.tochoices = this.TNK.tochoices.concat(
                CLASS_TREE[i][User.class]
              );
            }
          }
          this.TNK.show = -0.5;
        ...
        ///
        let reverse =
          (1 / UIRATIO) * CONST.OFFCAN * CONST.RESOLUTION * CONST.RESOLUTION;
        if (this.TNK.hide) {
          this.TNK.show = -0.5;
          let isIn = General["isMouse"](
            Global.winW - 30 / reverse,
            Global.winH -
              (-this.TNK.mDown + this.TNK.size * this.TNK.choices.length) /
                reverse,
            30 / reverse,
            (-this.TNK.mDown + this.TNK.size * this.TNK.choices.length) /
              reverse
          ...
          let isIn = General["isMouse"](
            Global.winW -
              (-this.TNK.mRight + this.TNK.size / 2 + this.TNK.hideWidth / 2) /
                reverse,
            Global.winH -
              (this.TNK.size / 2 +
                15 * reverse +
                this.TNK.hideHeight +
                this.TNK.mDown +
                (this.TNK.size + 2) * this.TNK.choices.length) /
                reverse,
        ...
        for (let i in this.TNK.choices) {
          ...
          let isIn =
            General["isMouse"](
              ...
              Global.winH -
                (-this.TNK.mDown + (c.height * (parseInt(i) + 1) - 5)) /
                  reverse,
              (c.width - 20) / reverse,
              (c.height - 20) / reverse
            ) &&
          ...
          ctx.drawImage(
            c,
            20 + (this.TNK.mRight - c.width - 20) * this.TNK.dshow,
            this.TNK.mDown - (c.height + 2) * (parseInt(i) + 1)
          );
        }
        ctx.globalAlpha = 0.7;
        ctx.drawImage(
          this.TNK.logo,
          20 +
            (this.TNK.mRight - gw / 2 - this.TNK.logo.width / 2 - 20) *
              this.TNK.dshow,
          -gw / 2 -
            15 * reverse -
            this.TNK.logo.height -
            this.TNK.mDown -
            (gw + 2) * this.TNK.choices.length
        );
      };
      ...
      this.messages = function() {
        ...
        for (let m of this.MES.mes) {
          if (m.img) {
            ...
            offy +=
              m.a > 1
                ? (m.can.height + 10) * m.da * CONST.RESOLUTION
                : (m.can.height + 10) * CONST.RESOLUTION;
            ctx.globalAlpha = a;
            ctx.restore();
            continue;
          }
      ...
      this.endScreen = function() {
        ...
        if (this.END.offy) {
          ...
          ctx.drawImage(
            this.END.title,
            -this.END.title.width / 2,
            -this.END.title.height - this.END.tank.height * 0.8 - 200 * invert
          );
          ctx.drawImage(
            this.END.tank,
            -this.END.tank.width,
            this.END.tank.height / 2 - 200 * invert
          );
          ctx.drawImage(
            this.END.enter,
            0,
            this.END.tank.height - this.END.enter.height / 4 - 200 * invert
          );
        }
      };
      ///
    ...
    General["doors"] = new (function() {
      ...
      this.update = function() {
        ...
        if (this.close == this.toClose) {
          return;
        }
        this.toClose += (this.close - this.toClose) * 0.04;
        if (this.toClose > 0.99) {
          this.toClose = 1;
        }
        if (this.toClose < 0.001) {
          this.toClose = 0;
        }
      };
      this.draw = function() {
        ...
        ///
        ctx.closePath();
        ctx.fillStyle = General.color.shade(
          "#ffffff",
          1 - (1 - this.toClose) / 10
        );
        ctx.fill();
    ...
    function getFps() {
      ...
      let toshow = Global.fps.reduce(function(t, n) {
        return t + n;
      });
    ...
    function Draw() {
      ...
      ///
      let sx = -User.gx * RATIO + Global.canW / 2,
        sy = -User.gy * RATIO + Global.canH / 2;
    ...
    function Loop() {
      ...
      if (Global.mouseDelay) {
        ...
        General["WS"].send(
          PROTO.encode("mousemove", {
            x: Math.min(
              0.5,
              Math.max(
                -0.5,
                (((Global.mouse_x - User.dvx / CONST.SMOOTH - Global.winW / 2) /
                  Game.screen) *
                  CONST.RESOLUTION) /
                  RATIO
              )
            ...
            y: Math.min(
              ...
              Math.max(
                -0.5,
                (((Global.mouse_y - User.dvy / CONST.SMOOTH - Global.winH / 2) /
                  (Game.screen * 0.5625)) *
                  CONST.RESOLUTION) /
                  RATIO
              )
        ...
        if (
          Global.mouse_x != Global.oldMouse_x ||
          Global.mouse_y != Global.oldMouse_y
        ) {
          User.DIFFDIR = 1;
          ...
          for (let CONSTRUC in data.Instances) {
            for (let OBJ in data.Instances[CONSTRUC]) {
              ...
              if (typeof inst[OBJ] === "undefined") {
                switch (CONSTRUC) {
                  case "Players":
                    inst[OBJ] = new Tank(obj.x, obj.y, obj.size, obj.color);
                    inst[OBJ].bot = obj.states[7];
                    break;
                  case "Objects":
                    inst[OBJ] = new Obj(obj.x, obj.y, obj.size, obj.type);
                    break;
                  case "Bullets":
                    inst[OBJ] = new Bullet(
                      obj.x,
  ...
  General["CHAT"] = (() => {
    ...
    function log(arr) {
      for (let data of arr) {
        ...
        log.innerHTML = data[0].length
          ? `<span style="color: #${splited[0]}">` +
            escapeHtml(name) +
            " : </span>"
          : '<span style="color:#ccc;font-weight:500">server : </span>';
