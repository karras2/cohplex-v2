(function(window) {
  ...
  function Buy(c, i, force = 0) {
    if (
      ...
      (POST.shop &&
        POST.shop[c] &&
        POST.shop[c][i] &&
        POST.shop[c][i].price <= UserData.coins)
    ) {
  ...
  for (let i in POST.shop.pets) {
    ...
    itDiv.children[0].onclick = function(itemId) {
      if (this.owned) {
        for (let i in PETS.children) {
          if (PETS.children[i].children && i != itemId)
            PETS.children[i].children[0].classList.remove("item-select");
        }
    ...
    itDiv.children[0].children[2].children[0].innerHTML = spacify(
      POST.shop.pets[i].price
    );
  ...
  window.SetPets = function(owned) {
    ...
    for (let item in owned) {
      ...
      div.children[0].innerHTML =
        '<span style="color: #aab; font-size: 1em;">Owned</span>';
