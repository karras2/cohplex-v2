  ...
  Req.onload = function() {
    ...
    if (UserData.own && UserData.own.pets) {
      ...
      if (UserData.own.pets[window.Pref.pet]) {
        ChosenPet = Pref.pet;
        document
          .getElementById("pets-zone")
          .children[ChosenPet].children[0].classList.toggle("item-select");
  ...
  var toggleLB = (() => {
    ...
    POST.leader.forEach((item, i) => {
      ...
      leader.classList.add(
        "leader",
        item.userKey == POST.key ? "isMe" : "notme"
      );
      let is = "";
