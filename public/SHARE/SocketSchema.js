(function(exports, platform) {
  ...
  var SCHEMA = {
    ...
    GameUpdate: {
      ...
      Objects: ["states", "shape", "hp", "x", "y", "size", "alpha"],
      Bullets: ["states", "type", "x", "y", "size", "color", "alpha", "dir"]
    },
    UiUpdate: {
      leader: ["xp", "name", "nameC", "team"]
    }
  };
  var toSTRING = {
    construc: ["Players", "Objects", "Bullets"],
    gamemode: ["ffa", "2team", "4team"],
    type: [
      "init",
      "kick",
      "keydown",
    ...
    shapes: ["sqr", "tri", "pnt", "alphaPnt", "alphaSqr", "alphaTri", "bull"],
    class:
      platform == "client"
        ? TanksConfig.list
        : require("./TanksConfig.js").list,
    color: [
      "green",
      "red",
      "yellow",
    ...
    read(type) {
      ...
      switch (type) {
        ...
        case "float32":
          ...
          break;
        case "str":
          c = this.dv.getUint8(this.cursor) * 2 + 1;
          break;
        case "str8":
          c = this.dv.getUint8(this.cursor) + 1;
          break;
      }
      this.cursor += c;
    ...
    read(type) {
      ...
      switch (type) {
        ...
        case "float32":
          ...
          break;
        case "str":
          c = 1 + this.buffer.readUInt8(this.cursor) * 2;
          break;
        case "str8":
          c = this.buffer.readUInt8(this.cursor) + 1;
          break;
      }
      this.cursor += c;
      ...
      return (type, data) => {
        ...
        switch (type) {
          ...
          case "Instance": {
            ...
            let id = INST.id;
            ENC.write(
              toBUFFER["construc"][construc],
              TYPE.GameUpdate.CONSTRUCTOR
            );
            ...
            for (let n of SCHEMA["GameUpdate"][construc]) {
              ...
              switch (n) {
                ...
                case "alpha": {
                  ENC.write(
                    Math.max(Math.min(parseInt(inset * 255), 255), 0),
                    Type
                  );
                  break;
                }
                ...
                case "xp": {
                  ...
                  ENC.write(
                    parseInt(inset / Math.pow(1000, exp)) * 10 + exp,
                    Type
                  );
                  break;
                }
                ...
                case "canDir": {
                  ...
                  ENC.write(
                    inset
                      .map(x =>
                        String.fromCharCode(
                          parseInt(((x + Math.PI) / (Math.PI * 2)) * 65535)
                        )
                      )
                      .join(""),
                    Type
          ...
          /////
          case "chatUpdate": {
            data = new Array(data.length * 2)
              .fill(0)
              .map((x, i) => data[parseInt(i / 2)][i % 2]);
    ...
    if (platform == "server") {
      return data => {
        ...
        switch (type) {
          ///
          case "init": {
            result.error = checkLength(data.byteLength, 25, 65)
              ? 0
              : "ERR_PACKET_LENGTH";
            if (result.error) {
              break;
            ...
            result.error = checkLength(result.data.key, 25, 25)
              ? 0
              : "ERR_BROKEN_KEY";
            break;
          }
          case "ping": {
            result.error = checkLength(data.byteLength, 1, 1)
              ? 0
              : "ERR_PACKET_LENGTH";
            break;
          }
          ...
          case "keydown":
          case "keyup": {
            result.error = checkLength(data.byteLength, 2, 2)
              ? 0
              : "ERR_PACKET_LENGTH";
            if (result.error) {
              break;
          ...
          case "mousemove": {
            result.error = checkLength(data.byteLength, 9, 9)
              ? 0
              : "ERR_PACKET_LENGTH";
            if (result.error) {
              break;
          ...
          ///
          case "chat": {
            result.error = checkLength(data.byteLength, 202, 202)
              ? 0
              : "ERR_PACKET_LENGTH";
            if (result.error) {
              break;
          ...
          case "com": {
            result.error = checkLength(data.byteLength, 52, 52)
              ? 0
              : "ERR_PACKET_LENGTH";
            if (result.error) {
              break;
        ...
        switch (type) {
          ...
          case "GameUpdate": {
            ...
            for (let n of SCHEMA.GameUpdate.Players) {
              ...
              switch (n) {
                case "states": {
                  result.data.User[n] = DEC.read(Type)
                    .toString(2)
                    .substr(1)
                    .split("")
                    .map(x => parseInt(x));
                ...
                case "canDir": {
                  ...
                  result.data.User[n] = deco.length
                    ? deco
                        .split("")
                        .map(
                          x =>
                            (x.charCodeAt(0) / 65535) * (Math.PI * 2) - Math.PI
                        )
                    : [];
                  break;
                }
              ...
              for (let n of SCHEMA["GameUpdate"][construc]) {
                ...
                switch (n) {
                  ...
                  case "xp": {
                    ...
                    obj[n] =
                      parseInt(raw / 10) +
                      toSTRING.xpExt[raw - parseInt(raw / 10) * 10];
                    break;
                  }
                  case "color": {
                  ...
                  case "canDir": {
                    ...
                    obj[n] = deco.length
                      ? deco
                          ...
                          .map(
                            x =>
                              (x.charCodeAt(0) / 65535) * (Math.PI * 2) -
                              Math.PI
                          )
                      : [];
                    break;
